    ...
    if args.level_all:
        ...
        for eps in range(num_runs):
            ...
            if not args.no_render:
                events = event.events()
            while not (
                events["closed"] or game.is_game_over or events["quit_to_main_menu"]
            ):
                ...
                collisions.resolve_all_collisions(
                    game.balls, game.holes, game.table_sides
                )
                ...
                if game.all_not_moving():
                    ...
                    while (
                        ...
                        if not args.no_render:
                            events = event.events()
                        game.cue.cue_is_active(
                            game, events, not args.no_render, debug=args.debug
                        )
                        curr_tries += 1
        ...
        if args.generate_stats:
            stats["mean_tries"] = (
                sum(num_tries_arr) / len(num_tries_arr)
                if len(num_tries_arr) != 0
                else 0
            )
        ...
        assert (
            args.level_x < config.num_levels and args.level_x >= 0
        ), "Invalid level chosen"
